...
data "template_cloudinit_config" "elk" {
  ...
  part {
    ...
    content      = <<-EOF
    #!/bin/bash
    echo 'export ELASTIC_VERSION="${var.elk_version}"' >> /opt/env_elk_version
    echo 'export ELASTIC_PASSWORD="${var.elastic_password}"' >> /opt/env_elastic_pass
    echo 'export LOGSTASH_INTERNAL_PASSWORD="${var.logstash_password}"' >> /opt/env_logstash_pass
    echo 'export KIBANA_SYSTEM_PASSWORD="${var.kibana_password}"' >> /opt/env_kibana_pass
    EOF
  }
  #second part
  part {
